...
const getZennArticles = async () => {
	const response = await fetch('https://zenn.dev/api/articles?username=hiro_torii&order=latest', {
		mode: 'cors'
	});
...
const getQiitaArticles = async () => {
	const response = await fetch('https://qiita.com/api/v2/items?query=user:hiro-torii', {
		mode: 'cors'
	});
	const jsonData = await response.json();
	const articles: Article[] = [];
